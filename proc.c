...
fork(void)
{
  ...
  // copyuvm() - copies the parent’s user-space memory (page tables) to the child.
  // If this fails, then it:
      // - Frees the allocated kernel stack
      // - Marks the process as UNUSED
      // - Returns `-1`
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        ...
        // free the child’s user address space (page tables and physical pages).
        // freevm tears down the page directory and frees user pages.
        freevm(p->pgdir);
        p->pid = 0;
        p->parent = 0;
        p->name[0] = 0;
...
int va_to_pa(int va){
  ...
  pde_t *pde;
  pde = &pgdir[PDX(va)];
  cprintf("Page Directory Index %p\n", PDX(va));
  cprintf("Value stored at Page Directory Index is %p\n", *pde);

  pte_t *pgt_base, *pgte;
  pgt_base = (pte_t*)P2V(PTE_ADDR(*pde));
  pgte = &pgt_base[PTX(va)];
  cprintf("Page Table Index %p\n", PTX(va));
  cprintf("Value stored at Page Table Index is %p\n", *pgte);

  pte_t pa = PTE_ADDR(*pgte);
  cprintf("Base of Physical address Page %p\n", pa);
